function g = matRad_gradFuncWrapper(w,dij,cst,options,robustObjInfo)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if exist('robustObjInfo')
    dROContainer = cell(size(robustObjInfo,1),1);
    for i = 1:length(dROContainer)
        dROContainer{i} = {matRad_backProjectionRobust(w, robustObjInfo{i,3}, options)}; % Added 4th Jan 2020. Y Xia.
    end
end
% Initializes delta
        ...
        for j = 1:numel(cst{i,6})
            ...
            if isempty(strfind(cst{i,6}(j).type,'constraint'))
                ...
                if strcmp(cst{i,6}(j).robustness,'none')
                    if i == 64
                        d_i = d{1}(cst{i,4}{1});    % d is a dose vector of all voxels in the ct.
                    end
                
                    d_i = d{1}(cst{i,4}{1});

                    delta{1}(cst{i,4}{1}) = delta{1}(cst{i,4}{1}) + matRad_gradFunc(d_i,cst{i,6}(j),d_ref);
                else   % Aded 4th Jan 2020.
                    if i == 64
                        d_i = d{1}(cst{i,4}{1});    % d is a dose vector of all voxels in the ct.
                    end
                
                    d_i = d{1}(cst{i,4}{1});
                    for roInd = 1:length(dROContainer)
                        d_iRO = dROContainer{roInd,1}{1,1}(cst{i,4}{1});
                        min_worst = min(d_i, d_iRO);
                        max_worst = max(d_i, d_iRO);
                        delta{1}(cst{i,4}{1}) = delta{1}(cst{i,4}{1}) + matRad_gradFuncRobust(d_i,cst{i,6}(j),d_ref,min_worst,max_worst);
                    end
                end
