function [resultGUI,info] = matRad_fluenceOptimizationRobust(dij,cst,pln,tagAngleEnergy,robustObjInfo)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
global matRad_global_d_ro;  % Added 4th Jan 2020. Y Xia.
...
matRad_global_d_ro              = NaN * ones(dij.numOfVoxels,1);    % 4th Jan 2020.
...
%      wInit       = wOnes * bixelWeight;  
     ...
     % Initial conditions 11, random between 0 to 2 * bixelWeight, 
     % Uniform distribution.
%      wInit = rand(numel(wOnes),1) * 2 * bixelWeight;
     % Initial conditions 12, normal distribution.
     % Mean = bixelWeight. Sigma = bixelWeight / 3.
     wmin = 0;
     wmax = 2 * bixelWeight;
     wInit = wmin + (wmax - wmin) * sum(rand(numel(wOnes),6),2)/6;
     
    % Initial conditions method 1.
%     wOnes = zeros(50147,1);
%     wOnes(24511:24511+2583) = 1;
%     wOnes(34469:34469+2568) = 1;
%      bixelWeight =  (doseTarget)/(dij.RBE * mean(dij.physicalDose{1}(V,:)*wOnes));
    % Initial conditions method 2.
%     bixelWeight = ones(size(doseTarget))* 1000.;
    % Initial conditions method 3.
%     bixelWeight = ones(size(doseTarget))* 2000.;
    % Initial conditions method 4.
%     bixelWeight = ones(size(doseTarget))* 500.;
    % Initial conditions method 5.
%     bixelWeight = ones(size(doseTarget))* 700.;
    % Initial conditions method 6.
%     bixelWeight = ones(size(doseTarget))* 1.;
     
%      wInit       = wOnes * bixelWeight;
    % 28th Feb 2020. Y Xia. Change the initial values for w.
%     wInit = wInitCustom;
...
funcs.objective         = @(x) matRad_objFuncWrapper(x,dij,cst,options,robustObjInfo);
funcs.constraints       = @(x) matRad_constFuncWrapper(x,dij,cst,options);
funcs.gradient          = @(x) matRad_gradFuncWrapper(x,dij,cst,options,robustObjInfo);
...
switch env
     case 'MATLAB'
        clearvars -global matRad_global_x matRad_global_d matRad_global_d_ro matRad_objective_function_value matRad_STRG_C_Pressed;
     case 'OCTAVE'
        clear     -global matRad_global_x matRad_global_d matRad_global_d_ro matRad_objective_function_value matRad_STRG_C_Pressed;           
end
